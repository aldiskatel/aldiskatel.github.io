...
(function() {
    ...
    wwgame.getLivingWolvesList = function() {
        return wwgame.makeListWherePlayerMatches(function(p) { return p.alive && p.role.role == ww.Roles.Pembunuh; });
    };
            ...
            if (wolves.length >= nonWolves.length) {
                ...
                if(allHunters) {
                    if(nonWolves.length == 1) {
                        gameOver = {evilWins:false, text:"Parity has been reached, the hunter rises up to kill the final pembunuh."};
                    }
                    else {
