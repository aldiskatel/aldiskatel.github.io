...
$(function() {
    ...
    ww.duskPlayerScreen.setupCurrentPlayerActions = function() {
        ...
        $("#roleInfo").html("Kamu adalah <b>" + wwhtml.GetRoleAndAttributes(curPlayer.role) + "</b>");
        $("#duskPlayerBg").removeClass();
        $("#duskPlayerBg").addClass("centerAlign " + curRole.imgClass);

        // Show known info
        var instructions = '';
        if (curRole == ww.Roles.Pembunuh) {
            if (wolves.length == 1) {
                instructions += "Hanya kamu yangpembunuh.";
            }
            else {
                if (wolves.length == 2) {
                    instructions += "Kamu adalah pembunuh along with ";
                }
                else {
        ...
        if (wwgame.maxNightActions === 0) {
            ...
            $("#doRoleAction").click(function() {
                if (isSuspectOnly) {
                    ...
                    else if (curRole == ww.Roles.Pembunuh) {
