...
(function() {
    ...
    wwhtml.GetViewText = function(curPlayer, viewedPlayer) {
        ...
        if (curPlayer.role.role == ww.Roles.Saksi) {
            viewFor = "seorang pembunuh";
            if (viewedPlayer.role.role == ww.Roles.Pembunuh) {
                gotHit = true;
            }
        }
        else if(curPlayer.role.role == ww.Roles.Sorcerer) {
            viewFor = "a seer";
            if (viewedPlayer.role.role == ww.Roles.Saksi) {
                gotHit = true;
            }
        }
        ...
        // Build text
        viewedText += gotHit ? " ADALAH " : " BUKAN ";
        viewedText += viewFor;
        viewedText += ".";
